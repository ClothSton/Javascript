 ...
 */
(function (window) {
    // IE8 미만의 브라우저에서 console 관련 오류 방지
    var method,
        noop = function () { },
        methods = [
...
(function (window, $, dews) {
    ...
    $.extend(true, dews, {
        ...
        string: {
            ...
            /**
             * 문자열에 개인정보 마스크를 적용합니다.
             * @param {string} str 마스크를 적용할 대상 문자열
             * @param {string} mask 마스크 문자열
             * @param {boolean} predefined 통제환경 유무
             * @returns {string} 마스크가 적용된 문자열
             */
            secureMask: function (str, mask, predefined) {
                var result = '',
                    idx = 0,
                    maskChar,
                    rules = {
                ...
                if ((typeof str === 'string' || typeof str === 'number') && typeof mask === 'string') {
                    ...
                    var isPredefinedMask = false;

                    // 통제환경 코드를 사용할 경우
                    if(predefined) {
                        var dispMask = dews.getSecureMaskFormat(mask);

                        if(dispMask) {
                            // 새로운 마스크 포멧 변경되었을 경우 처리 로직 변경
                            mask = dispMask;

                            isPredefinedMask = true;
                        }
                        else {
                            // 통제환경에 없는 코드일 경우
                            return str;
                    ...
                    if(arrMask.length > 2) {
                        // 개인정보 마스크일 경우 처리
                        /**
                         * 개인정보 마스크일 경우 처리(구분자 : ,(콤마))
                         * arrMask[0] : 마스크 적용 형식(앞->뒤: F, 뒤->앞: B, 기준문자: 특수문자(@,- 등))
                         * arrMask[1] : 시작인덱스 또는 기준문자시 앞쪽 길이
                         * arrMask[2] : 종료인덱스 또는 기준문자시 뒤쪽 길이
                         * arrMask[3] : 특수문자 포함 처리여부 (생략가능: 생략시 불포함)
                         */
                        var maskGubun = "";
                        if(arrMask[0]) maskGubun = arrMask[0];
                        var startPos = 0;
                        if(arrMask[1]) startPos = parseInt(arrMask[1], 10);
                        var endPos = 0;
                        if(arrMask[2]) endPos = parseInt(arrMask[2], 10);
                        var isSpecialChar = "F";
                        if(arrMask[3]) isSpecialChar = arrMask[3];
                        ...
                        if(maskGubun === "F" || maskGubun === "B") {
                            if(maskGubun === "B") {
                                // 뒤에서부터 시작일 경우 문자열 뒤집기
                                orgValue = orgValue.split("").reverse().join("");
                            }
                            if(orgValue !== "") {
                                var resultMaskValue = this.chgSecureMask(orgValue, orgValue.split(" ").join(""), startPos, endPos, isSpecialChar);

                                if(resultMaskValue !== "") {
                                    chgValue = resultMaskValue;
                                }

                                if(maskGubun === "B") {
                                    // 뒤집은 문자 다시 원래대로 뒤집기
                                    chgValue = chgValue.split("").reverse().join("");
                                }
                            }
                        }
                        else if(pattern_spc.test(maskGubun)) {
                            // 기준이 특수문자가일 경우
                            if(orgValue !== "") {
                                var gubunPos = orgValue.indexOf(maskGubun);
                                if (gubunPos > -1) {
                                    var prevStr = orgValue.substring(0, gubunPos);
                                    prevStr = prevStr.split("").reverse().join("");
                                    var nextStr = orgValue.substring(gubunPos + 1, orgValue.length);

                                    var prevResultMaskValue = this.chgSecureMask(prevStr, prevStr.split(" ").join(""), 0, (startPos - 1), isSpecialChar);
                                    if (prevResultMaskValue !== "") {
                                        prevStr = prevResultMaskValue;
                                        prevStr = prevStr.split("").reverse().join("");
                                    }
                                    var nextResultMaskValue = this.chgSecureMask(nextStr, nextStr.split(" ").join(""), 0, (endPos - 1), isSpecialChar);
                                    if (nextResultMaskValue !== "") {
                                        nextStr = nextResultMaskValue;
                                    }

                                    chgValue = prevStr + maskGubun + nextStr;
                                }
                            }
                        }
            ...
            /**
             * 문자열에 *을 적용합니다
             * @param {string} oStr 원본 문자열
             * @param {string} tStr 공백 제거한 문자열
             * @param {int} sPos 마스크 시작 인덱스
             * @param {int} ePos 마스크 종료 인덱스
             * @param {string} isSpec 특수문자 포함 여부(T/F)
             * @returns {string} * 마스크가 적용된 문자열
             */
            chgSecureMask: function(oStr, tStr, sPos, ePos, isSpec) {
                var pattern_spc = /[\{\}\[\]\/?.,;:|\)*~`!^\-_+┼<>@\#$%&\'\"\\\(\=]/gi; // 특수문자
                ...
                for(var i=0; i<tStr.length; i++) {
                    if(!tStr[chgIdx]) break;
                    else if(chgIdx >= sPos && chgIdx <= ePos) {
                        if(isSpec === "F" && tStr[chgIdx].match(pattern_spc)) {
                            chgValue += tStr[chgIdx];
                            ePos++;
                        }
                        else {
                            chgValue += "*";
                ...
                // 공백이 있는 문자열이었을 경우 처리
                if(oStr.indexOf(" ") > -1) {
                    var arrTrimStr = oStr.split(" ");
                    if(arrTrimStr.length > 0) {
                        var trimIdx = 0;
                        for(var t=0; t<arrTrimStr.length; t++) {
                            if(arrTrimStr[t] === "") addTrimValue += " ";
                            else {
                                if(t !== 0) addTrimValue += " ";
                                addTrimValue += chgValue.substring(trimIdx, (trimIdx+arrTrimStr[t].length));

                                trimIdx += arrTrimStr[t].length;
                            }
                        }

                        if(addTrimValue.length > 0 && addTrimValue[0] === " ") addTrimValue = addTrimValue.substring(1, addTrimValue.length);

                        if(addTrimValue !== "") chgValue = addTrimValue;
                    }
                }
        ...
        number: {
            ...
            format: function (number, format) {
                ...
                if (decimalIndex != -1) {
                    ...
                    if (fraction[1]) {
                        fraction = this.round(number, Math.abs(fraction[1]));
                    } else {
                        fraction = fraction[0];
                    }
                ...
                // 간헐적으로 ,###.## 또는 -,###.##과 같은 형태로 출력되는 현상이 있으므로 설정
                var reg = /^(-?),/;
                if (reg.test(number)) {
            ...
            round: function(value, precision) {
                var sign =  value < 0 ? "-" : "", // 부호
                    point = isNaN(precision = Math.abs(precision)) ? 0 : precision, // 자릿수
                    int =  parseInt(value = Math.abs(+value || 0).toFixed(point)) + ""; // 정수부

                value = sign + int.replace(/(\d{3})(?=\d)/g, "$1") + (point ? Math.abs(value - int).toFixed(point).slice(2) : "");
                value = value.toString().split('e');
                value = (+(value[0] + 'e' + (value[1] ? (+value[1] - point) : -point)));

                return value.toFixed(point);
            }
        },
        /**
         * guid 값 설정시 uuid로 변환해 줍니다.
         * @param guid 변경할 guid 값
         * @return {*} uuid 값
         */
...
(function (window, $, dews) {
    ...
    $.extend(true, dews, {
        ...
        _environments: {
            ...
            useLog: false,
            /**
             * jquery.maskedinput 로드 여부
             */
            loadJqueryMaskedinput: false
        },
        /**
...
(function ($, dews) {
                        ...
                        if (type.toLowerCase() === 'data' || type.toLowerCase() === 'api') {
                            ...
                            if (data.hasOwnProperty("state")) {
                                if (data.state === 'error') {
                                    ...
                                    defer.resolveWith(this, [data.data, textStatus, jqXHR, data]);
                                    return;
                                }
                            }
                        }
    ...
    $.extend(true, dews, {
        ...
        url: {
            ...
            /**
             * 뷰 화면의 URL을 가져옵니다.
             * @param {string} module 모듈코드
             * @param {string} viewName 화면 이름(.html 생략)
             */
            getViewUrl: function (module, viewName) {
                if (!module || !viewName) {
                    throw '잘못된 값이 입력되었습니다.'
                }
                ...
                return dews.string.format('/view/{0}/{1}', module, viewName);
            }
        }
    });
